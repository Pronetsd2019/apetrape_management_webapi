...
/**
 * Get Part Find Requests by Status Endpoint
 * GET /part_find/get_by_status.php?status[]=pending&status[]=quoted
 * or
 * GET /part_find/get_by_status.php?status=pending,quoted
 */
...
// Check if the user has permission to read part finds
...
try {
    ...
    // Supports both ?status[]=pending&status[]=quoted and ?status=pending,quoted
    $statusArray = [];
    
    if (isset($_GET['status'])) {
        if (is_array($_GET['status'])) {
            // Array format: ?status[]=pending&status[]=quoted
            $statusArray = $_GET['status'];
        } else {
            // Comma-separated format: ?status=pending,quoted
    ...
    if (empty($statusArray)) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'At least one status is required. Use ?status[]=pending&status[]=quoted or ?status=pending,quoted'
        ]);
        exit;
    }
    ...
    $conditions[] = "pfr.status IN ($statusPlaceholders)";
    ...
    logException('part_find_get_by_status', $e);
