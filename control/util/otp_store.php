...
/**
 * Generates a 6-digit OTP and stores it (hashed) for password reset.
 *
 * @return array{otp_code:string, otp_id:int, expires_at:string}
 */
function createPasswordResetOtp(PDO $pdo, int $userId, string $channel, string $destination): array {
    $otp = (string)random_int(100000, 999999);
    $codeHash = password_hash($otp, PASSWORD_DEFAULT);
    ...
    // Remove old unconsumed password_reset OTPs for this user
    try {
        $cleanup = $pdo->prepare("
            DELETE FROM user_otps
            WHERE user_id = ? AND purpose = 'password_reset' AND consumed_at IS NULL AND expires_at < NOW()
        ");
        $cleanup->execute([$userId]);
    } catch (Throwable $e) {
        logException('otp_store_cleanup', $e, ['user_id' => $userId]);
