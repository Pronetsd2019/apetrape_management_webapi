...
/**
 * Mobile Notifications Get Unread Endpoint
 * GET /mobile/v1/notifications/get_unread.php?limit=20&offset=0
 * Requires JWT authentication - returns unread notifications for the authenticated user
 */
...
try {
    ...
    // Get unread notifications with full notification details
    $stmt = $pdo->prepare("
        SELECT 
    ...
    logException('mobile_notifications_get_unread', $e);
    http_response_code(500);
    echo json_encode([
        'success' => false,
        'error' => 'Server error',
        'message' => 'An error occurred while retrieving unread notifications. Please try again later.',
        'error_details' => 'Error retrieving unread notifications: ' . $e->getMessage()
    ]);
} catch (Exception $e) {
    logException('mobile_notifications_get_unread', $e);
    http_response_code(500);
