...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, supplier_id FROM contact_persons WHERE id = ? AND supplier_id = ?");
    $stmt->execute([$contact_person_id,$supplierId]);
    $existing = $stmt->fetch();
    ...
    if (isset($supplierId)) {
        ...
        $params[] = $supplierId;
        $supplier_id = $supplierId; // Update for email check
    }
    if (isset($input['name'])) {
        $update_fields[] = "name = ?";
        $params[] = $input['name'];
    ...
    logException('supplier_contact_persons_update', $e);
