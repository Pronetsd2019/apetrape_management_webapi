...
$required_fields = ['name', 'physical_address', 'city_id'];
...
try {
    ...
    if ($create_new_contact) {
        ...
        if (!empty($new_contact['email'])) {
            ...
            $stmt->execute([$new_contact['email'], $supplierId]);
            if ($stmt->fetch()) {
        ...
        if ($contact_person['supplier_id'] != $supplierId) {
            http_response_code(400);
    ...
    logException('supplier_stores_create', $e);
